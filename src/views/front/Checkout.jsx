...
function Checkout(params){
  ...
  const [lodaingCartId,setLodaingCartId] = useState(null);
  const [lodaingProductId,setLodaingProductId] = useState(null);
  const productModalRef = useRef(null);

  const {register,handleSubmit,formState: { errors }} =useForm({ mode:"onChange"});// 即時驗證
  ...
  return(
    <div className="container">
          ...
          <tr key = {product.id}>
            ...
            <td>
              <div className="btn-group btn-group-sm">
                ...
                disabled={lodaingProductId === product.id}>
                  {
                    lodaingProductId === product.id ?(
                      <RotatingLines
                      color='grey'
                      width={80}
                      height={16}/>
                ...
                <button type="button" className="btn btn-outline-danger" onClick={() =>addCart(product.id)}
                  disabled = {lodaingCartId === product.id}>
                  {
                    lodaingCartId === product.id ? (
                      <RotatingLines
                      color='grey'
                      width={80}
                      height={16}/>
      ...
      <div className="text-end mt-4">
        <button type="button" className="btn btn-outline-danger" onClick={()=>delAll()}>
          清空購物車
        </button>
      </div>
          ...
          <tr key={cartItem.id}>
            ...
            <td>
              <div className="input-group input-group-sm mb-3">
                <input type="number" className="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing" defaultValue={cartItem.qty} onChange={(e)=> updateCart(cartItem.id,cartItem.product_id,Number(e.target.value))}/>
